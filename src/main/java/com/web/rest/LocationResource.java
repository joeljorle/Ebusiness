...
/**
 * REST controller for managing {@link com.domain.Location}.
 */
@RestController
@RequestMapping("/api")
public class LocationResource {
    ...
    public ResponseEntity<List<LocationDTO>> getAllLocations(Pageable pageable, @RequestParam(required = false) String filter) {
        if ("category-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where category is null");
            return new ResponseEntity<>(locationService.findAllWhereCategoryIsNull(),
                    HttpStatus.OK);
        }
        if ("product-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where product is null");
            return new ResponseEntity<>(locationService.findAllWhereProductIsNull(),
                    HttpStatus.OK);
        }
        if ("evenement-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where evenement is null");
            return new ResponseEntity<>(locationService.findAllWhereEvenementIsNull(),
                    HttpStatus.OK);
        }
        if ("profile-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where profile is null");
            return new ResponseEntity<>(locationService.findAllWhereProfileIsNull(),
                    HttpStatus.OK);
        }
        if ("tour-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where tour is null");
            return new ResponseEntity<>(locationService.findAllWhereTourIsNull(),
                    HttpStatus.OK);
        }
        if ("tourgroup-is-null".equals(filter)) {
            log.debug("REST request to get all Locations where tourgroup is null");
            return new ResponseEntity<>(locationService.findAllWhereTourgroupIsNull(),
                    HttpStatus.OK);
        }
