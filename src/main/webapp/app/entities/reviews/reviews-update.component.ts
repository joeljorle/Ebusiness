...
export class ReviewsUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ reviews }) => {
      if (!reviews.id) {
        ...
        reviews.createdat = today;
        reviews.updatedat = today;
      }
  ...
  updateForm(reviews: IReviews): void {
    this.editForm.patchValue({
      ...
      text: reviews.text,
      createdat: reviews.createdat ? reviews.createdat.format(DATE_TIME_FORMAT) : null,
      updatedat: reviews.updatedat ? reviews.updatedat.format(DATE_TIME_FORMAT) : null,
      categoryId: reviews.categoryId,
      productId: reviews.productId,
      evenementId: reviews.evenementId
    });
