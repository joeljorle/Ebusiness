...
export class FilesUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ files }) => {
      if (!files.id) {
        ...
        files.createdat = today;
        files.updatedat = today;
      }
  ...
  updateForm(files: IFiles): void {
    this.editForm.patchValue({
      ...
      createdat: files.createdat ? files.createdat.format(DATE_TIME_FORMAT) : null,
      updatedat: files.updatedat ? files.updatedat.format(DATE_TIME_FORMAT) : null
    });
  }
