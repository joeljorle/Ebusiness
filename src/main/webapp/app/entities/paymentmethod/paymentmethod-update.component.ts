...
export class PaymentmethodUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.activatedRoute.data.subscribe(({ paymentmethod }) => {
      if (!paymentmethod.id) {
        ...
        paymentmethod.createdat = today;
        paymentmethod.updatedat = today;
      }
  ...
  updateForm(paymentmethod: IPaymentmethod): void {
    this.editForm.patchValue({
      ...
      createdat: paymentmethod.createdat ? paymentmethod.createdat.format(DATE_TIME_FORMAT) : null,
      updatedat: paymentmethod.updatedat ? paymentmethod.updatedat.format(DATE_TIME_FORMAT) : null
    });
  }
